...
Log.printLine("/////"+p.getTasks().size()+"----"+p.outerCut()+"==multi==="+p.innerCut());
	...
	private List<Partition> bipartition(List<Task> tasks){
	...
	/*public List<Partition> TabuPartition(double compute,double bw){
		if(getTasks().size()<50){
			Partition partition=new Partition(new ArrayList<Task>(getTasks()));
			List<Partition> best=new ArrayList<Partition>();
			best.add(partition);
			return best; 
		}
		int num=(int)Math.ceil((double)(getTasks().size()/20.0));
		List<Partition> pList=new ArrayList<>();
		int size=(getTasks().size()/num==0)?(getTasks().size()/num):(getTasks().size()/num+1);
	
		for(int i=0;i<num;i++){
			List<Task> tasks=new ArrayList<>();
			for(int j=i*size;j<Integer.min(size*(i+1),getTasks().size());j++)
				tasks.add(getSortedTasks().get(j));					
			pList.add(new Partition(tasks));
		}	
		...
		for(Task task:getTasks()){
			for(Partition p:pList){//Log.printLine(task.getCloudletId()+"-"+task.getPartition());
				if(p.isNeibor(task.getPartition()))
					moveGains.put(task,task.getPartition().movegain(task, p));
			}
		}
		int stop=0;
		int seed=0;
